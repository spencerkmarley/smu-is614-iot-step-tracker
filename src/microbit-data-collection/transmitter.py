...
def on_forever2():
    ...
    x_val = Math.round(input.acceleration(Dimension.X) / 10)
    y_val = Math.round(input.acceleration(Dimension.Y) / 10)
    z_val = Math.round(input.acceleration(Dimension.Z) / 10)
    gyro_x_val = SENMPU6050.gyroscope(axisXYZ.X, gyroSen.RANGE_250_DPS)
    gyro_y_val = SENMPU6050.gyroscope(axisXYZ.Y, gyroSen.RANGE_250_DPS)
    gyro_z_val = SENMPU6050.gyroscope(axisXYZ.Z, gyroSen.RANGE_250_DPS)
...
def on_forever3():
    global accel
    while sendData:
        accel = "" + str(Math.round(input.acceleration(Dimension.X) / 10)) + "," + ("" + str(Math.round(input.acceleration(Dimension.Y) / 10))) + "," + ("" + str(Math.round(input.acceleration(Dimension.Z) / 10))) + "," + ("" + str(Math.round(SENMPU6050.gyroscope(axisXYZ.Z, gyroSen.RANGE_250_DPS) / 10)))
